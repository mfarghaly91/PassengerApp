...
router.get("/routes/:id/stations", async (req, res): Promise<void> => {
  ...
  if (!params.success) { res.status(400).json({ error: params.error.message }); return; }
  const stations = await db.select().from(stationsTable).where(eq(stationsTable.routeId, params.data.id)).orderBy(stationsTable.order);
  res.json(stations);
});
...
router.post("/routes/:id/stations", authenticate, requireRole("admin"), async (req, res): Promise<void> => {
  ...
  if (!params.success) { res.status(400).json({ error: params.error.message }); return; }
  const parsed = AddStationBody.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [station] = await db.insert(stationsTable).values({ ...parsed.data, routeId: params.data.id }).returning();
...
router.patch("/routes/:id/stations/:stationId", authenticate, requireRole("admin"), async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [updated] = await db.update(stationsTable).set(parsed.data)
